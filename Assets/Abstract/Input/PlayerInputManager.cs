...
public class PlayerInputManager : MonoBehaviour, PlayerControls.IDefaultPlayerActions
{
    ...
    public Vector3 rawLookInput; //Raw directional input translated to world space
    ...
    /// <summary>
    /// Calls Update for movement and look behaviours
    /// </summary>
    private void Update()
    {
        ApplyMovement();

        if (lastViableControllerInput.x != 0 || lastViableControllerInput.y != 0) //Do not move if no direction. Deadzone in input applies here
        {
    ...
    private void ApplyMovement()
    {
        ...
        if (!playerCharacterController.isGrounded) 
            playerCharacterController.Move(Physics.gravity * Time.deltaTime); //apply world gravity downwards, scaled for consistency
    }

    /// <summary>
    /// Applies the current look input to the 
    /// </summary>
    ...
    public void OnWalk(InputAction.CallbackContext context)
    {
        ...
        if (context.performed)
        {
            ...
            rawInputDirection = new Vector3(input.x, 0, input.y); //Vector3 Y is Up
        }
    }
    ...
    /// 
    /// Functionality for sticks differ slightly to the mouse
    /// It persistently applies as opposed to as and when its triggered
    /// </summary>
    /// <param name="context"></param>
    public void OnStickLook(InputAction.CallbackContext context)
    {
        if (context.canceled) //Stick Released
        { 
            lastViableControllerInput = Vector2.zero; //setting to zero invalidates the check on update
            return;
        }
        if (context.performed) //Stick Pushed in direction
        {
            Vector2 stickDelta = context.ReadValue<Vector2>() * mouseSpeed * Time.deltaTime; //deltatime scaling could (maybe even should) probably go somewhere else
            if (stickDelta.x != 0) //only update x or y as nescesarry
    ...
    public void OnInteract(InputAction.CallbackContext context)
    {
        ...
        //Have another script called Selector that will handle the logic i.e. raycasts to detect things and call
        //The select method
    }
